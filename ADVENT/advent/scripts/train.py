...
def train(gpu, args):
    ...
    rank = args.rank * cfg.TRAIN.NUM_GPUS + gpu
    ...
    print('Model loaded to rank %d' % rank)
    ...
    # UDA TRAINING
    train_domain_adaptation(model, source_loader, target_loader, cfg, gpu, rank)
...
if __name__ == '__main__':
    ...
    cfg_from_file(args.cfg)
    args.world_size = cfg.TRAIN.NUM_GPUS * cfg.TRAIN.NUM_NODES
    os.environ['MASTER_ADDR'] = str(cfg.TRAIN.MASTER_ADDR)
    os.environ['MASTER_PORT'] = str(cfg.TRAIN.MASTER_PORT)

    torch.multiprocessing.spawn(train, nprocs=cfg.TRAIN.NUM_GPUS, args=(args,))
    #main()
